...
export const sendPass = async (
  ...
  if (view) {
    ...
    if (newUser) {
      ...
      welcome = [
        `¿Te olvidaste la contraseña? No te preocupes, a continuación te pasamos los nuevos datos de acceso a ${Names[0].productName}`,
      ];
    }
    ...
    return new Promise((resolve, reject) => {
      ejs.renderFile(
        path.join('views', 'emails', 'Templates', 'ForgotPass.ejs'),
        datos,
        async (err, data) => {
          if (err) {
            console.error(err);
